...
int main(int argc, char *argv[]) {
	...
	// 0 => sequential, 1 => block, 2 => cyclic, 3 => block cyclic
	uint64_t chunkSize = 64;// default block size for block cyclic policy
	
	if (argc != 1 && argc > 5) {
		std::printf("use: %s N [T] [P [C]]\n", argv[0]);
	...
	if (argc > 1) {
		...
		if (argc >= 5){
			chunkSize = std::stol(argv[4]);
		}
	}

	// Create the barrier
	std::barrier syncPoint(threadNum, [](){ });//std::cout << "Ended phase" << std::endl; });
	...
	auto blockCyclicWavefront = [&](std::vector<double> &M, const uint64_t &N, uint64_t threadId,
		uint64_t threadNum, uint64_t chunkSize){
		for (uint64_t k = 1; k < N; ++k) {
			for (uint64_t j = 0; j < N-k; j += chunkSize * threadNum){
				uint64_t start = j + threadId * chunkSize;
				uint64_t end = std::min(j + (threadId + 1) * chunkSize, N-k);
				for (uint64_t i = start; i < end; ++i) {
					work(k, i, M, N);
				}
			}
	...
	//std::printf("Estimated Block-Parallel Compute time ~ %f (ms)\n", expected_totaltime/(1000.0 * threadNum));
	...
	if (policy == 0){
		...
		for (uint64_t i = 0; i < threadNum; i++)
			threads.emplace_back(
				blockWavefront, std::ref(M), N, i, threadNum, chunkSize
			);
	} else if (policy == 2){ // cyclic policy
		for (uint64_t i = 0; i < threadNum; i++)
			threads.emplace_back(
				cyclicWavefront, std::ref(M), N, i, threadNum, chunkSize
			);
	} else if (policy == 3){
		for (uint64_t i = 0; i < threadNum; i++)
			threads.emplace_back(
				blockCyclicWavefront, std::ref(M), N, i, threadNum, chunkSize
			);
	} else {
